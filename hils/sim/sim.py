import os, socket, time, threading, queue, csv, pathlib, sys, subprocess
sys.path.append('/app')
...
def setup_network_delay():
    ...
    if NETWORK_DELAY_MS > 0:
        try:
            ...
            # 遅延を設定
            subprocess.run(['sudo', 'tc', 'qdisc', 'add', 'dev', 'eth0', 'root', 'netem',
                           'delay', f'{NETWORK_DELAY_MS}ms'], check=True)
...
log.writerow([
    ...
    'timeout','deadline_miss_ms','sent_value','received_result'
])
