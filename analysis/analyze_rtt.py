...
def load_sim_data(log_dir):
    ...
    # 必要な列が存在するかチェック
    required_cols = ['step_id', 't_sim_send_ns', 't_sim_recv_ns', 't_act_recv_ns', 't_act_send_ns']
...
def calculate_rtt_metrics(df):
    ...
    # 処理時間（アクチュエーター内）
    valid_df['act_processing_us'] = (valid_df['t_act_send_ns'] - valid_df['t_act_recv_ns']) / 1000
...
def create_plots(df, output_dir=None):
    ...
    # 1. 時系列RTTプロット
    ax1 = axes[0, 0]
    ax1.plot(df['time_s'], df['e2e_rtt_us'], 'b-', alpha=0.7, linewidth=0.5, label='E2E RTT')
    ax1.plot(df['time_s'], df['sim_to_act_us'], 'g-', alpha=0.7, linewidth=0.5, label='Sim->Act')
    ax1.plot(df['time_s'], df['act_to_sim_us'], 'r-', alpha=0.7, linewidth=0.5, label='Act->Sim')
    ...
    ax2.axvline(df['e2e_rtt_us'].mean(), color='red', linestyle='--', label=f'Mean: {df["e2e_rtt_us"].mean():.1f}μs')
    ax2.axvline(df['e2e_rtt_us'].median(), color='green', linestyle='--', label=f'Median: {df["e2e_rtt_us"].median():.1f}μs')
...
def main():
    ...
    parser.add_argument('--output', '-o', type=str, help='Output directory for plots (default: log directory)')
